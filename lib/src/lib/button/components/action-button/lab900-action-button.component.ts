...
export class Lab900ActionButtonComponent {
  private readonly _action$ = new ReplaySubject<ActionButton>();
  ...
  @Input({ required: true })
  public set action(action: ActionButton) {
  ...
  public constructor() {
    ...
    this.disabled$ = stream.pipe(map(([a, d]) => readPropValue(a.disabled, d)));
