...
export class Lab900TableCellValueComponent<T = any> implements AfterViewInit {
  @Input()
  ...
  public readonly cell$: Observable<TableCell<T>> = this._cell$
    .asObservable()
    .pipe(shareReplay(1));
  public readonly data$: Observable<T> = this._data$
    .asObservable()
    .pipe(shareReplay(1));
  ...
  public constructor() {
    ...
    this.cellValue$ = streams.pipe(
      map(([cell, data]) => cellFormatter(cell, data))
    );
    this.maxWidth$ = streams.pipe(
      map(([cell, data]) => this.maxWidth(cell, data))
    );
  }
